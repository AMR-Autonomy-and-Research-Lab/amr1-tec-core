...
import { getReadableNameForElement as getReadableNameForElement2, cju } from "@tscircuit/circuit-json-util";
...
var getCollidableBounds = (collidable) => {
  ...
  return getBoundsOfPcbElements2([collidable]);
};
  ...
  for (const segmentA of pcbTraceSegments) {
    ...
    for (const obj of nearbyObjects) {
      ...
      if (isCircular) {
        ...
        const distance3 = segmentToCircleMinDistance(
          { x: segmentA.x1, y: segmentA.y1 },
          { x: segmentA.x2, y: segmentA.y2 },
...
function checkViasOffBoard(circuitJson) {
  ...
  if (board.width === void 0 || board.height === void 0) return [];
...
var getSourceTraceIdsFromPcbTrace = (pcbTraceSourceTraceId) => {
  if (!pcbTraceSourceTraceId) return [];
  if (!pcbTraceSourceTraceId.includes("__")) return [pcbTraceSourceTraceId];
  return pcbTraceSourceTraceId.split("__").filter((part) => part.startsWith("source_trace_"));
};
function checkSourceTracesHavePcbTraces(circuitJson) {
  ...
  for (const sourceTrace of sourceTraces) {
    ...
    const hasPcbTrace = pcbTraces.some(
      (pcbTrace) => getSourceTraceIdsFromPcbTrace(pcbTrace.source_trace_id).includes(
  ...
  for (const trace of pcbTraces) {
    ...
    for (let i = 0; i < trace.route.length - 1; i++) {
      ...
      for (let j = 0; j < boardPoints.length; j++) {
        ...
        const distance3 = segmentToSegmentMinDistance2(
          segmentStart,
          segmentEnd,
          edgeStart,
          edgeEnd
...
import {
  cju as cju3,
  getBoundsOfPcbElements as getBoundsOfPcbElements4,
...
function doPcbElementsOverlap(elem1, elem2) {
  ...
  const bounds1 = getBoundsOfPcbElements3([elem1]);
  const bounds2 = getBoundsOfPcbElements3([elem2]);
...
function checkPcbComponentOverlap(circuitJson) {
  ...
  for (const [componentId, componentData] of componentMap) {
    if (componentData.elements.length > 0) {
      componentData.bounds = getBoundsOfPcbElements4(componentData.elements);
    }
  }
  ...
  for (let i = 0; i < componentsWithElements.length; i++) {
    for (let j = i + 1; j < componentsWithElements.length; j++) {
      ...
      if (!doBoundsOverlap2(comp1.bounds, comp2.bounds)) {
        continue;
      }
